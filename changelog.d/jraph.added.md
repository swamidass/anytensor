...
``anytensor.tree`` (``jax.tree`` API plus ``batch``/``unbatch``). ``jraph.batch``
is ``tree.batch``; GraphsTuple owns sender offsets via ``__tree_batch__`` /
``__tree_unbatch__``. Public names cover every entry in official
``jraph.__all__``. Hypothesis parity vs upstream jraph (when JAX is installed)
includes batch/pad, GraphNetwork, the model zoo (GAT with self-edges, not
