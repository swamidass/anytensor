...
| **`jax.jit` + `repeat` / `partition_softmax`** | `jnp.repeat` needs a **static** repeat count or `total_repeat_length`. `partition_softmax` requires `sum_partitions` (`shape(logits)[0]`); `num_segments` is `shape(partitions)[0]`, not an argument. |
