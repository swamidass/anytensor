...
| **`jax.jit` + `repeat` / `partition_softmax`** | `jnp.repeat` needs a **static** repeat count or `total_repeat_length`. `partition_softmax` always requires `num_segments` (same name as `segment_softmax`) and `sum_partitions` (`shape(logits)[0]`). |
