...
- ONNX recipes live in the opt-in `anytensor.export` subpackage (not in
  ...
  conversion to reuse with `segment_*`; `cache` is a decorator (GraphNetwork apply), a reentrant
  context, and ``enable``/``disable``; dict of dicts with ids at ``cache["partition"]``; ``purge`` drops one tensor. Hetero and
