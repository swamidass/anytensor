...
- NaN utilities: `is_nan` / `is_finite` / `is_inf` (aliases `isnan` / `isfinite` / `isinf`), `fill_nan` (alias `nan_fill`), Array API `nan_to_num`, element-wise `equal_nan`. Chose verb-first `fill_nan` over `nan_fill`; kept the latter as alias.
